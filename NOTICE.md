# Paper assets notice

The images in `assets/model-overview.png`, `assets/encoder-architecture.png`,
`assets/featuremask-architecture.png`, `assets/lights4-architecture.png`,
`assets/main-results-table.png`, `assets/wsj0-generalization-tables.png`, and
`assets/librispeech-generalization-tables.png` reproduce selected figures and
tables from:

> Chen Jiang, Dai Gao, Sirui Wang, Chengxuan Zou, Jie Liu. “Lightweight speech
> enhancement with state-space model and depthwise separable convolution.”
> *Digital Signal Processing*, 174 (2026), 105987.
> https://doi.org/10.1016/j.dsp.2026.105987

These paper figures are included for scholarly description and attribution.
They are not covered by the MIT License that applies to the source code.
Copyright in the published article and its figures remains with the applicable
rights holder(s).

The original publisher-formatted PDF is intentionally not distributed in this
repository. Use the DOI link above to access the version of record.
